...
#else // FFTW_TRANSPOSE
    CMPI_SETUP_TRANSFORM_BACKWARD(np0_,(fftw_complex*)&val[ibase]);
    // #pragma omp parallel for
    // for ( int i = 0; i < np0_; i++ )
    // {
...
#else // FFTW_TRANSPOSE
    CMPI_SETUP_TRANSFORM(np0_,(fftw_complex*)&val[ibase]);
    // #pragma omp parallel for
    // for ( int i = 0; i < np0_; i++ )
    // {
...
void FourierTransform::init_lib(void)
{
  ...
  fwplany = CMPI_BUILD_PLAN(rank, n, howmany, (fftw_complex*)&aux[0],
                                    inembed, istride, idist,
                                    (fftw_complex*)&aux[0], onembed,
                                    ostride, odist, -1, FFTW_ALGO);
  bwplany = CMPI_BUILD_PLAN_BACKWARD(rank, n, howmany, (fftw_complex*)&aux[0],
                                    inembed, istride, idist,
                                    (fftw_complex*)&aux[0], onembed,
                                    ostride, odist, 1, FFTW_ALGO);
#endif // FFTW_TRANSPOSE
